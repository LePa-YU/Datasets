...
READ_PATH = os.path.abspath(os.path.join(os.path.dirname( __file__ ), '..', 'EECS4462','Webpages'))
WRITE_PATH = os.path.abspath(os.path.join(os.path.dirname( __file__ ), '..', 'EECS4462'))
...
def read_files(READ_PATH):
    ...
    for file in filenames:
        filename = os.path.join(READ_PATH, file) #READ_PATH+'\\'+file
        file_location = os.path.join(WRITE_PATH, file.replace(" ", "_").split('.html')[0]) #WRITE_PATH+'\\'+file.replace(" ", "_").split('.html')[0]+"\\"
        text_file_name = os.path.join(file_location,file.replace(" ", "_").split('.')[0]+'_content.txt')
        link_file_name = os.path.join(file_location,file.replace(" ", "_").split('.')[0]+'_links.txt')
        print(file)
        print(file_location)
    ...
    for links in BeautifulSoup(file, features="html.parser", parse_only=SoupStrainer('a')):
        if links.has_attr('href'):
            ...
            if link.endswith('pdf') or link.endswith('txt') or link.endswith('xlsx') :
                ...
                with open(os.path.join(file_location,filename),"wb") as pdf:
                    for chunk in r.iter_content(chunk_size=1024):
                        # writing one chunk at a time to pdf file
                        if chunk:
                            pdf.write(chunk)
    # EBL: Needed to read the file again, was not working for me otherwise
    file = codecs.open(html_file, 'r', 'utf-8')
